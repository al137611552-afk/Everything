...
namespace QuickPanel {
 ...
 public static class Services {
  ...
  public static List<Entry> SystemApps(){var root=Environment.GetFolderPath(Environment.SpecialFolder.System);var rows=new[]{new[]{"控制面板","control.exe","control controlpanel kongzhimianban kzmb"},new[]{"记事本","notepad.exe","notepad jishiben jsb"},new[]{"计算器","calc.exe","calculator calc jisuanqi jsq"},new[]{"画图","mspaint.exe","paint huatu ht"},new[]{"任务管理器","Taskmgr.exe","taskmanager renwuguanliqi rwglq"},new[]{"命令提示符","cmd.exe","cmd minglingtishifu"}};return rows.Where(r=>File.Exists(Path.Combine(root,r[1]))).Select(r=>new Entry{Name=r[0],Target=Path.Combine(root,r[1]),Detail="Windows 系统工具",Kind="应用",Glyph="\uE8A5",Alias=r[2]}).ToList();}
  ...
  public static string EsArguments(string query,string exportPath=null){return "-argv -n 80 -txt -utf8-bom -no-header -no-footer -no-double-quote -timeout 3000 "+(exportPath==null?"":"-export-txt "+Quote(exportPath)+" ")+Quote("<"+query+">");}
  public static async Task<List<Entry>> Files(Settings settings,string query,CancellationToken ct,Action<string> progress=null,int startupTimeoutMs=30000){var es=FindEs(settings);if(es==null)throw new IOException("尚未配置 Everything 命令行工具 es.exe。请在设置中选择它，并运行 Everything。");if(String.IsNullOrWhiteSpace(query))return new List<Entry>();
   string instance="";if(String.Equals(Path.GetFullPath(es),Path.Combine(SearchRuntime.Tools,"es.exe"),StringComparison.OrdinalIgnoreCase)){instance=SearchRuntime.Ensure();} var exportPath=Path.Combine(Path.GetTempPath(),"Quick-search-"+Guid.NewGuid().ToString("N")+".txt");try{var startup=Stopwatch.StartNew();while(true){ct.ThrowIfCancellationRequested();using(var p=new Process()){p.StartInfo=new ProcessStartInfo(es,instance+EsArguments(query,exportPath)){UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true,StandardOutputEncoding=Encoding.UTF8,StandardErrorEncoding=Encoding.UTF8};p.Start();using(ct.Register(()=>{try{p.Kill();}catch{}})){var stdout=p.StandardOutput.ReadToEndAsync();var stderr=p.StandardError.ReadToEndAsync();var wait=Task.Run(()=>p.WaitForExit());if(await Task.WhenAny(wait,Task.Delay(6000,ct))!=wait){try{p.Kill();}catch{}await wait;ct.ThrowIfCancellationRequested();throw new IOException("Everything 查询超时，请检查客户端与索引状态。");}ct.ThrowIfCancellationRequested();string output=await stdout;await stderr;if(p.ExitCode==8){if(startup.ElapsedMilliseconds>=startupTimeoutMs)throw new IOException("Everything 启动连接超时：通信窗口仍未就绪（代码 8）。请在设置中打开 Everything，检查实例是否正常运行。");if(progress!=null)progress("正在连接 Everything…就绪后自动搜索，无需再次输入");await Task.Delay(250,ct);continue;}if(p.ExitCode!=0)throw new IOException("Everything 未返回结果（代码 "+p.ExitCode+"）。请确认客户端正在运行，且支持 IPC。");if(!File.Exists(exportPath))throw new IOException("Everything 未生成搜索结果文件，请检查 CLI 版本。");return ParseFiles(File.ReadAllText(exportPath,new UTF8Encoding(false,true)));}}}}finally{if(File.Exists(exportPath))File.Delete(exportPath);}
  }
