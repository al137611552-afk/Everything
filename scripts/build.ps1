...
$destination = Join-Path $root 'outputs\Quick-v0.3.5'
...
& (Join-Path $framework 'csc.exe') /nologo /target:winexe /platform:x64 /optimize+ "/win32icon:$root\assets\Quick.ico" "/win32manifest:$source\app.manifest" "/out:$destination\Quick.exe" $refs $sources
if ($LASTEXITCODE -ne 0) { throw 'Compilation failed' }
Copy-Item -LiteralPath (Join-Path $source 'Main.xaml') -Destination $destination -Force

Copy-Item -LiteralPath (Join-Path $root 'assets\Quick.ico') -Destination $destination -Force
